...
export class CategoriesService {
    ...
    return this.categoriesModel
      ...
      .then((categ) => {
        if (categ instanceof Categories)
          return Responder({ status: HttpStatusCode.Created, data: categ });
        else
          return Responder({ status: HttpStatusCode.BadRequest, data: null });
      })
      .catch((err) => {
        log(err);
        return Responder({
  ...
  async createThematic(
    createCategoryDto: CreateThematicFormationDto,
  ): Promise<ResponseServer> {
    const { thematic, description } = createCategoryDto;
    return this.thematicModel
      .create({
        thematic,
        description,
        status: 1,
      ...
      .then((categ) => {
        if (categ instanceof Thematiques)
          return Responder({ status: HttpStatusCode.Created, data: categ });
        else
          return Responder({ status: HttpStatusCode.BadRequest, data: null });
      })
  ...
  async updateCategorieFormations(
    idCategory: number,
    ...
    if (Object.keys(updateCategoryDto).length > 0) {
        ...
        .then((categ) => {
          if (categ instanceof Categories) {
            return categ
              .update({
                ...updateCategoryDto,
                ...(updateCategoryDto.hasOwnProperty('id_thematique')
                  ? { ThematicId: updateCategoryDto.id_thematique }
                  : {}),
              })
              .then((_) =>
                Responder({ status: HttpStatusCode.Ok, data: categ }),
              )
              .catch((err) =>
                Responder({
                  status: HttpStatusCode.InternalServerError,
            ...
            return Responder({
              ...
              data: `La categorrie:${idCategory} n'a pas ete retrouver dans le système `,
            });
          }
        })
        .catch((error) =>
      ...
      return Responder({
        ...
        data: 'The body should not be empty !',
      });
    }
  }

  async updateThematicFormations(
    idThematic: number,
    updateCategoryDto: UpdateThematicFormationDto,
  ): Promise<ResponseServer> {
    if (Object.keys(updateCategoryDto).length > 0) {
      return this.thematicModel
        .findOne({
        ...
        .then((categ) => {
          if (categ instanceof Categories) {
            ...
            return Responder({
              status: HttpStatusCode.NotFound,
              data: `La Thematique : ${idThematic} n'a pas ete retrouver dans le système `,
            });
          }
        })
        .catch((error) =>
