...
export class WebrtcGatewayService implements OnGatewayInit, OnGatewayConnection, OnGatewayDisconnect {
    ...
    async handleCreateSendTransport(@ConnectedSocket() client: Socket) {
        const transportInfo = await this.mediasoupService.createSendTransport(client.id);
    ...
    async handleConnectTransport(
        @MessageBody() data: { transportType: 'send' | 'recv'; dtlsParameters: any },
        @ConnectedSocket() client: Socket
    ) {
        const transport = this.mediasoupService.getTransport(client.id, data.transportType); // ✅ Correction ici

        if (!transport) {
            this.logger.error(`No ${data.transportType} transport found for client ${client.id}`);
            return;
        }

        await transport.connect({ dtlsParameters: data.dtlsParameters });

        this.logger.log(`${data.transportType} transport connected for client ${client.id}`);
    ...
    async handleProduce(
        ...
        try {
            ...
            this.logger.log(`Producer created for client ${client.id}, kind: ${data.kind}`);
        } catch (err) {
