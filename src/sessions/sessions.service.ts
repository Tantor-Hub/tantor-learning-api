...
export class SessionsService {
  ...
  async createSessionFullStep(
    createSessionDto: CreateSessionFullStepDto,
    user: IJwtSignin,
    ...
    try {
      ...
      const session = await this.handleCreationOfSession(
        createSessionDto,
        transaction,
        user,
      );
      ...
      const survey = await this.handleCreationOfSurvey(
        { questions, id_session: sessionId },
        user,
        transaction,
      );
      ...
      return Responder({
        ...
        data: { session: session.data, survey: survey.data },
      });
    } catch (error) {
      await transaction.rollback();
      return Responder({
  ...
  async uploadDocumentToSessionDTO(
    user: IJwtSignin,
    uploadDocumentDto: UploadDocumentToSessionDto,
    group: string,
  ): Promise<ResponseServer> {
    const {
      id_session,
    ...
    try {
      ...
      const userSession = await this.hasSessionStudentModel.findOne({
        where: { id_sessionsuivi: id_session, id_stagiaire: user.id_user },
      });
      if (!userSession)
        return Responder({
          status: HttpStatusCode.NotFound,
  ...
  async GetDocumentsByGroup(
    ...
    try {
      ...
      if (!student)
        return Responder({
          status: HttpStatusCode.NotFound,
          data: `Student not found with id: ${idStudent}`,
        });
      ...
      if (!session)
        return Responder({
          status: HttpStatusCode.NotFound,
          data: `Session not found with id: ${idSession}`,
        });
      ...
      if (!sessionStudent)
        return Responder({
          status: HttpStatusCode.BadRequest,
          data: `The student is not associated to this session: ${idSession}`,
        });
      const { id } = sessionStudent.toJSON();
      return this.documentModel
        .findAll({
          include: [
            {
        ...
        .then((pref) =>
          Responder({
            status: HttpStatusCode.Ok,
            data: { length: pref.length, list: pref },
          }),
        )
        .catch((err) =>
          Responder({ status: HttpStatusCode.InternalServerError, data: err }),
  ...
  async listOfLearnerByIdSession(idsession: number): Promise<ResponseServer> {
    ...
    return this.hasSessionStudentModel
      .findAndCountAll({
        subQuery: false,
        where: {
          id_sessionsuivi: idsession,
  ...
  async listOfLearnerByConnectedFormateur(
    user: IJwtSignin,
  ): Promise<ResponseServer> {
    const list = await this.sessionModel.findAll({
      where: { id_superviseur: { [Op.contains]: [user.id_user] } },
      attributes: ['id', 'id_superviseur'],
    });
  ...
  async listAllSessionsByOwnAndStatus(
    user: IJwtSignin,
    status: number,
  ): Promise<ResponseServer> {
    const { id_user } = user;
    StagiaireHasSession.belongsTo(Formations, { foreignKey: 'id_formation' });
    StagiaireHasSession.belongsTo(SessionSuivi, {
      foreignKey: 'id_sessionsuivi',
    });
  ...
  async listAllSessionsByOwnAsFormateur(
    user: IJwtSignin,
  ): Promise<ResponseServer> {
    const { id_user } = user;
  ...
  async listAllSessionsByIdInstructor(
    idinstructor: number,
  ): Promise<ResponseServer> {
    SessionSuivi.belongsTo(Categories, { foreignKey: 'id_category' });
    SessionSuivi.belongsTo(Formations, { foreignKey: 'id_formation' });
  ...
  async getSessionById(idsession: number): Promise<ResponseServer> {
    try {
      ...
      return this.sessionModel
        ...
        .then((session) => {
          if (session instanceof SessionSuivi)
    ...
    try {
      ...
      return this.sessionModel
        ...
        .then((session) => {
          return Responder({
            status: HttpStatusCode.Ok,
            data: { length: session.length, list: session },
          });
        })
        .catch((_) => {
          return Responder({
  ...
  async applyToSession(
    applySessionDto: CreateSessionPaiementDto,
    user: IJwtSignin,
  ): Promise<ResponseServer> {
    const { id_session, payment, roi_accepted, responses_survey } =
      applySessionDto;
    const { id_user } = user;
    const transaction = await this.sequelize.transaction();
    ...
    try {
      if (!roi_accepted) {
        ...
        return Responder({
          status: HttpStatusCode.BadRequest,
          data: 'Vous devez accepter le ROI pour postuler à cette session !',
        });
      }

      const student = await this.usersModel.findOne({
        where: { id: id_user, status: 1 },
      });
      if (!student) {
        await transaction.rollback();
        return Responder({
          status: HttpStatusCode.NotFound,
          data: 'Targeted user not found',
        });
      }
      ...
      if (!(inst instanceof SessionSuivi)) {
        ...
        return Responder({
          status: HttpStatusCode.BadRequest,
          data: "La session ciblée n'a pas été retrouvée !",
        });
      }

      const data = inst.toJSON() as any;
      const id_formation = data.id_formation as number;
      const designation = data.designation as string;
      const prix = Number(data.prix ?? 0);
      const nb_places_disponible = Number(data.nb_places_disponible ?? 0);
      const { titre } = ((data as any).Formation ?? {}) as { titre?: string };
      ...
      if (!isNew) {
        ...
        return Responder({
          status: HttpStatusCode.BadRequest,
          data: 'Vous êtes déjà inscrit à cette session',
        });
      }
      ...
      if (nb_places_disponible <= 0) {
        ...
        return Responder({
          ...
          data: "Il n'y a plus de place disponible !",
        });
      }
        ...
        switch (payment.method) {
          case 'CARD':
            ...
            const base = String(card.callback).endsWith('/')
              ...
              : String(card.callback) + '/';
            const priceInCents = Math.round(Number(prix) * 100);
            ...
            if (stripeResp.code !== 200) {
              ...
              return Responder({
                status: HttpStatusCode.BadRequest,
                data: 'Erreur lors de la création du paiement Stripe',
              });
            }
          ...
          case 'CPF':
            ...
            return Responder({
              status: HttpStatusCode.Created,
              data: 'Méthode de paiement CPF non prise en charge',
            });
          ...
          default:
            ...
            return Responder({
              status: HttpStatusCode.BadRequest,
              data: 'Méthode de paiement non prise en charge',
            });
        }
      }
  ...
  async getListePrestation(): Promise<ResponseServer> {
    return Responder({
      status: HttpStatusCode.Ok,
      data: { length: typesprestations.length, list: typesprestations },
    });
  }
  async getListeRealnce(): Promise<ResponseServer> {
    return Responder({
      status: HttpStatusCode.Ok,
      data: { length: typesrelances.length, list: typesrelances },
    });
  }
  async getListeActions(): Promise<ResponseServer> {
    return Responder({
      status: HttpStatusCode.Ok,
      data: { length: typesactions.length, list: typesactions },
    });
  }
  async handleCreationOfSession(
    createSessionDto: any,
    transaction: any,
    user: IJwtSignin | null,
    ...
    const s_on = this.allServices.parseDate(date_session_debut);
    const e_on = this.allServices.parseDate(date_session_fin);
    ...
    const designation_start = this.allServices.formatDate({
      dateString: s_on as any,
    });
    const designation_end = this.allServices.formatDate({
      dateString: e_on as any,
    });
    ...
    try {
      const { id_category, prix, type_formation } = form.toJSON();
      ...
      const formation = await this.sessionModel.create(
        {
          ...
          prix: prix > 0 ? this.allServices.calcTotalAmount(prix) : 0,
          initial_price: prix,
          type_formation,
    ...
    try {
      ...
      if (!(survey instanceof Survey)) {
        return {
          ...
          data: 'Échec de la création du questionnaire.',
        };
      }
      ...
      for (const question of questions) {
        const { is_required, options, titre, type_question, description } =
          question;
  ...
  async createSeance(
    addSeanceSessionDto: AddSeanceSessionDto,
  ): Promise<ResponseServer> {
    const {
      id_session,
      piece_jointe,
  ...
  async deleteseance(id_seance: number) {
    try {
      const session = await this.seancesModel.findOne({
        where: { id: id_seance },
      });
      if (!session)
        return Responder({
  ...
  async gatAllSessionsByThematicAndCategory(
    idThematic: number,
    idCategory: number,
  ): Promise<ResponseServer> {
    SessionSuivi.belongsTo(Categories, { foreignKey: 'id_category' });
  ...
  async updateSession(
    ...
    if (Object.keys(updateSessionDto).length <= 0)
      return Responder({
        status: HttpStatusCode.BadRequest,
        data: 'Le corps de cette requête ne devrait pas être vide !',
      });
    return this.sessionModel
      .findOne({
        where: {
      ...
      .then((inst) => {
        if (inst instanceof SessionSuivi) {
          ...
          return Responder({
            status: HttpStatusCode.NotFound,
            data: `La session n'a pas été retrouvée [id]:${idSession}`,
          });
        }
      })
      .catch((_) =>
    ...
    return this.coursModel
      ...
      .then((inst) => {
        if (inst instanceof Cours) {
          ...
          return Responder({
            status: HttpStatusCode.NotFound,
            data: `Le Cours n'a pas été retrouvée [id] :${id_user}`,
          });
        }
      })
      .catch((_) =>
